...
// 6. Página con el objeto personas en el script:
// Pregunta el nombre de la persona. Si coincide con uno de los 7 nombres en el objeto, muestra los datos en un div
// centrado. Incluye:
// Foto (picture)
// Nombre (h1)
// Atributos (edad, email, dirección)
// Fondo con el color de ojos (eyeColor).
// Si no existe, muestra un mensaje.

let nombre = window.prompt(`Introduzca su nombre: `)

if (!personas[name].includes(nombre)) {
    window.alert(`No hay ningún nombre que coincida`)
} else {
    document.getElementById(`id`).innerHTML = personas.nombre.picture, personas.nombre.name, personas.nombre.age, personas.nombre.email, personas.nombre.address
}
